...
#if DEBUG == 1
    cout << "read " << input_smarts.size()
	 << " SMARTS strings from " << line_count << " lines " << endl;
    for( unsigned int ii = 0 ; ii < input_smarts.size() ; ++ii )
      cout << ii << " : " << input_smarts[ii].first
	   << " -> " << input_smarts[ii].second << endl;
    cout << "there were " << smarts_sub_defn.size() << " vector bindings"
	 << endl;
    for( unsigned int ii = 0 ; ii < smarts_sub_defn.size() ; ++ii )
      cout << ii << " : " << smarts_sub_defn[ii].first
	   << " -> " << smarts_sub_defn[ii].second << endl;
#endif
